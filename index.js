...
export default {
  async fetch(request, env, ctx) {
    ...
    const sec13f = params.get("sec13f"); // ← NEW: dedicated 13F/SEC route with correct User-Agent
    ...
    try {
        ...
        try {
          const { results } = await env.DB.prepare(
            `SELECT ticker, name, issuer, asset_class, index_name,
                    net_assets, coverage_status, has_nport
             FROM etf_master
             ORDER BY ticker ASC`
          ).all();
      ...
      if (url.pathname === '/api/etf-snapshot') {
        ...
        const etfRow = await env.DB.prepare(
          `SELECT series_id, name FROM etf_master WHERE ticker = ?`
        ).bind(sym).first();
        ...
        return new Response(JSON.stringify({ ticker: sym, snapshots: results }), {
      ...
      if (url.pathname === '/api/etf-holdings-stored') {
        ...
        // Get the target month — use provided or latest available
      ...
      if (url.pathname === '/api/etf-overlap') {
        ...
        const [etfA, etfB] = await Promise.all([
          env.DB.prepare(`SELECT ticker, name, series_id FROM etf_master WHERE ticker = ?`).bind(symA).first(),
          env.DB.prepare(`SELECT ticker, name, series_id FROM etf_master WHERE ticker = ?`).bind(symB).first(),
        ]);
        ...
        return new Response(JSON.stringify({
          etf_a: { ticker: symA, name: etfA.name, report_month: monthA.report_month },
          etf_b: { ticker: symB, name: etfB.name, report_month: monthB.report_month },
      ...
      if (url.pathname === '/api/etf-compare') {
        ...
        const results = await Promise.all(symbols.map(async sym => {
          const etf = await env.DB.prepare(
            `SELECT ticker, name, issuer, asset_class, index_name, series_id, net_assets, coverage_status
          ...
          return {
            ...
            coverage_status: etf.coverage_status,
            snapshot:        snapshot || null,
            top10:           top10.results || [],
      ...
      if (url.pathname === '/api/etf-exposure') {
        ...
        const month = latestMonth.report_month;
        ...
        if (stockTicker) {
          const rows = await env.DB.prepare(`
            ...
            LIMIT ?
          `).bind(month, stockTicker, `%${stockTicker}%`, topN).all();
          ...
          const agg = await env.DB.prepare(`
            ...
            WHERE h.report_month = ?
              ...
              AND h.weight_pct > 0
          `).bind(month, stockTicker, `%${stockTicker}%`).first();
          ...
          const trend = await env.DB.prepare(`
            ...
            LIMIT 3
          `).bind(stockTicker, `%${stockTicker}%`).all();
      ...
      if (url.pathname === '/api/universe-changes') {
        ...
        const months = await env.DB.prepare(
          `SELECT DISTINCT report_month FROM fund_holdings_monthly
        ...
        if (monthList.length < 2) {
          return new Response(JSON.stringify({
            error: 'insufficient_data',
            message: 'Need at least 2 months of holdings data across the universe.',
            months_available: monthList.length
          }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        const issuerClause = (scope === 'issuer' && issuer)
          ? `AND m.issuer LIKE '%${issuer.replace(/'/g, "''")}%'`
          : '';
        ...
        const newPositions = await env.DB.prepare(`
          SELECT
            ...
            c.security_ticker,
            c.asset_cat,
            COUNT(DISTINCT c.ticker)  as fund_count,
            SUM(c.weight_pct)         as total_weight,
            AVG(c.weight_pct)         as avg_weight
          FROM fund_holdings_monthly c
          JOIN etf_master m ON m.ticker = c.ticker
          WHERE c.report_month = ?
            ${issuerClause}
            AND c.weight_pct > 0
            AND NOT EXISTS (
              SELECT 1 FROM fund_holdings_monthly p
              JOIN etf_master mp ON mp.ticker = p.ticker
              WHERE p.report_month = ?
                AND (p.cusip = c.cusip OR p.security_name = c.security_name)
                AND p.weight_pct > 0
                ${issuerClause.replace(/m\./g, 'mp.')}
            )
          GROUP BY c.security_name, c.cusip
          ORDER BY fund_count DESC, total_weight DESC
          LIMIT ?
        `).bind(currentMonth, previousMonth, limit).all();
        ...
        const exitedPositions = await env.DB.prepare(`
          SELECT
            ...
            p.security_ticker,
            p.asset_cat,
            COUNT(DISTINCT p.ticker)  as fund_count,
            SUM(p.weight_pct)         as total_weight,
            AVG(p.weight_pct)         as avg_weight
          FROM fund_holdings_monthly p
          JOIN etf_master m ON m.ticker = p.ticker
          WHERE p.report_month = ?
            ${issuerClause}
            AND p.weight_pct > 0
            AND NOT EXISTS (
              SELECT 1 FROM fund_holdings_monthly c
              JOIN etf_master mc ON mc.ticker = c.ticker
              WHERE c.report_month = ?
                AND (c.cusip = p.cusip OR c.security_name = p.security_name)
                AND c.weight_pct > 0
                ${issuerClause.replace(/m\./g, 'mc.')}
            )
          GROUP BY p.security_name, p.cusip
          ORDER BY fund_count DESC, total_weight DESC
          LIMIT ?
        `).bind(previousMonth, currentMonth, limit).all();

        const topIncreases = await env.DB.prepare(`
          SELECT
            c.security_name,
            c.cusip,
            c.security_ticker,
            ...
            AVG(c.weight_pct - p.weight_pct)  as avg_change
          FROM fund_holdings_monthly c
          JOIN fund_holdings_monthly p
            ON p.report_month = ?
            AND (p.cusip = c.cusip OR p.security_name = c.security_name)
        ...
        const topDecreases = await env.DB.prepare(`
          SELECT
            c.security_name,
            c.cusip,
            c.security_ticker,
        ...
        return new Response(JSON.stringify({
          ...
          new_positions:    newPositions.results    || [],
          exited_positions: exitedPositions.results || [],
          top_increases:    topIncreases.results    || [],
          top_decreases:    topDecreases.results    || []
        }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      // ── NEW: dedicated SEC 13F proxy route ──
      if (sec13f) {
        return await handleSec13F(sec13f, secUa, corsHeaders);
      }
...
// ── NEW: Dedicated 13F/SEC proxy — correct User-Agent, only allows sec.gov ──
async function handleSec13F(encodedUrl, secUa, corsHeaders) {
  let targetUrl;
  try {
    targetUrl = decodeURIComponent(encodedUrl);
  } catch {
    return json({ error: "Invalid SEC URL encoding" }, 400, corsHeaders);
  }
  if (!targetUrl.includes("sec.gov")) {
    return json({ error: "Only sec.gov URLs are permitted on this route" }, 403, corsHeaders);
  }
  const res = await secFetch(targetUrl, secUa);
...
// ── NEW: SEC-specific proxyText — always sends correct SEC User-Agent ──
