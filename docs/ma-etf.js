...
let entityIconAbortController = null;
...
// ── Bond detection ────────────────────────────────────────────
// A2: detect fixed income via asset_type weight (>50%).
// Confirmed D1 values for fixed income: DBT, ABS-MBS, ABS-O, STIV.
// Equity codes for reference: EC, DE, EP.
// If asset_type is missing/unpopulated for all rows → returns false → equity layout.
const _FI_ASSET_TYPES = new Set(['DBT','ABS-MBS','ABS-O','STIV']);

// A2: human-readable labels for SEC N-PORT asset_cat codes
function _assetCatLabel(code) {
  const map = {
    'DBT':     'Bond',
    ...
    'STIV':    'Short-term',
    'EC':      'Equity',
    'DE':      'Depositary receipt',
    'EP':      'Equity (preferred)',
    'LON':     'Loan',
    'RA':      'Repurchase agreement',
    'DIR':     'Direct investment',
    'OTHER':   'Other',
  };
...
function isBondHoldings(holdings) {
  ...
  let fiWeight = 0, totalWeight = 0;
  for (const h of holdings) {
    const w = getWeight(h);
    if (w <= 0) continue;
    totalWeight += w;
    const at = (h.asset_type || h.assetType || h.assetCat || h.asset_cat || '').toUpperCase().trim();
    if (_FI_ASSET_TYPES.has(at)) fiWeight += w;
  }
  if (totalWeight === 0 || fiWeight === 0) return false;
  return (fiWeight / totalWeight) > 0.5;
}
...
// A4: freshness badge HTML — green/amber/red
function renderFreshnessChip(status, asOfDate) {
  // Fix 1: suppress entirely when status is unknown/absent — blank is more honest than "Unknown"
  if (!status || status === 'unknown') return '';
  const labels = {
    current: { text: 'Current', cls: 'chip-fresh' },
    aging:   { text: 'Aging',   cls: 'chip-aging' },
    stale:   { text: 'Stale',   cls: 'chip-stale' }
  };
  const entry = labels[status];
  if (!entry) return '';
  const title = asOfDate ? `Data as of ${asOfDate}` : '';
  return `<span class="ma-chip ${entry.cls}"${title ? ` title="${title}"` : ''}>${entry.text}</span>`;
}

function renderCoverageChip(depth) {
  // Fix 2: suppress directory_only and null — "Directory" is internal terminology
  const labels = {
    single_month: { text: '1 Month',     cls: 'chip-single' },
    multi_month:  { text: 'Multi-Month', cls: 'chip-multi' },
    seasoned:     { text: 'Seasoned',    cls: 'chip-seasoned' }
  };
  if (!depth || !labels[depth]) return '';
  const { text, cls } = labels[depth];
  return `<span class="ma-chip ${cls}" title="Holdings depth: ${text}">${text}</span>`;
}

function renderEtfBadge(meta) {
  if (!meta) return '';
  const status = meta.freshness?.status;
  const asOf   = meta.freshness?.asOfDate;
  const depth  = meta.coverage?.depth;
  if (!status || status === 'unknown') return '';
  const title = asOf ? ` title="Data as of ${asOf}"` : '';
  if (status === 'stale') {
    return `<span class="ma-chip chip-stale"${title}>Stale</span>`;
  }
  const depthSuffix = depth === 'seasoned'   ? ' · 6+ months'
                    : depth === 'multi_month' ? ' · 2+ months'
                    : depth === 'single_month'? ' · 1 month'
                    : '';
  if (status === 'aging') {
    const agingSuffix = depth === 'seasoned'   ? ' · 6+mo'
                      : depth === 'multi_month' ? ' · 2+mo'
                      : depth === 'single_month'? ' · 1mo'
                      : '';
    return `<span class="ma-chip chip-aging"${title}>Aging${agingSuffix}</span>`;
  }
  return `<span class="ma-chip chip-fresh"${title}>Current${depthSuffix}</span>`;
}

function _etfFreshnessBadge(freshness, asOf) {
  const dateStr = asOf ? asOf.slice(0, 10) : '—';
  if (freshness === 'current') {
    return `<span style="font-size:9px;padding:1px 6px;border-radius:3px;
      background:rgba(34,197,94,0.15);color:var(--green,#22c55e);font-weight:600;
      white-space:nowrap;">Current · ${dateStr}</span>`;
  }
  if (freshness === 'aging') {
    return `<span style="font-size:9px;padding:1px 6px;border-radius:3px;
      background:rgba(245,158,11,0.15);color:var(--amber,#f59e0b);font-weight:600;
      white-space:nowrap;" title="Holdings data may be outdated">
      ⚠ ${dateStr} · Holdings data may be outdated</span>`;
  }
  return `<span style="font-size:9px;padding:1px 6px;border-radius:3px;
    background:rgba(239,68,68,0.15);color:var(--red,#ef4444);font-weight:600;
    white-space:nowrap;" title="Holdings data is significantly outdated — verify against SEC EDGAR">
    ✕ ${dateStr} · Holdings data is significantly outdated — verify against SEC EDGAR</span>`;
}
  ...
  if (!etfs || etfs.length === 0) {
    ...
    etfs = etfs.map(e => {
      ...
      return {
        ...
        dataFreshness:  e.dataFreshness  || null,  // A4
      };
    });
  }
...
function _fundRowHtml(e, activeTicker, clickHandler) {
  ...
  const hasDeep      = (e.coverageStatus || '').toLowerCase() === 'deep';
  const netAssetsStr = e.netAssets ? fmtCompact(e.netAssets) : null;

  // v2: freshness + coverage chips from meta=fc; fall back to A4 badge
  let badgeHtml = '';
  if (e.meta) {
    badgeHtml = renderFreshnessChip(e.meta.freshness.status, e.meta.freshness.asOfDate);
  } else if (hasDeep) {
    // A4 legacy badge fallback
    const freshness = e.dataFreshness || 'current';
    if (freshness === 'current') {
      badgeHtml = `<span style="font-size:9px;padding:1px 5px;border-radius:3px;
      ...
      badgeHtml = `<span style="font-size:9px;padding:1px 5px;border-radius:3px;
        ...
        font-weight:600;flex-shrink:0;" title="Holdings data may be outdated">⚠</span>`;
    } else {
      badgeHtml = `<span style="font-size:9px;padding:1px 5px;border-radius:3px;
        background:rgba(239,68,68,0.15);color:var(--red,#ef4444);
        font-weight:600;flex-shrink:0;" title="Holdings data is significantly outdated">✕</span>`;
    }
  }
...
async function openEtfHoldings() {
  ...
  let rawEtfs = null;
  try {
    const res = await fetch(`${MY_WORKER_URL}/api/etf-list?meta=fc`);
...
function _renderLauncherFundList() {
  ...
  if (searchVal) {
    ...
    if (deepOnly) {
      visible = visible.filter(e => (e.coverageStatus || '').toLowerCase() === 'deep');
    }
  }
  ...
  const legendHtml = `<div style="font-size:9px;color:var(--dim);padding:4px 8px 6px;line-height:1.5;border-bottom:1px solid var(--border);margin-bottom:4px;">
    <span style="font-weight:600;color:var(--text);">?</span>
    Freshness: <span class="ma-chip chip-fresh" style="font-size:8px;padding:1px 5px;">Current</span> ≤90d &nbsp;
    <span class="ma-chip chip-aging" style="font-size:8px;padding:1px 5px;">Aging</span> 91–180d &nbsp;
    <span class="ma-chip chip-stale" style="font-size:8px;padding:1px 5px;">Stale</span> 180+d
    &nbsp;|&nbsp; Coverage:
    <span class="ma-chip chip-single" style="font-size:8px;padding:1px 5px;">1 Month</span>
    <span class="ma-chip chip-multi" style="font-size:8px;padding:1px 5px;">Multi-Month</span>
    <span class="ma-chip chip-seasoned" style="font-size:8px;padding:1px 5px;">Seasoned</span> 6+ mo
  </div>`;
  list.innerHTML = legendHtml + visible.map(e => _fundRowHtml(e, null, 'onEtfFundRowClick')).join('');
}
...
window.onEtfFundRowClick = function(ticker) {
  ...
  closeModal();
  // Clear launcher DOM so legend chips don't persist invisibly in #mc
...
async function onEtfSelectChange(ticker) {
  ...
  if (entityIconAbortController) entityIconAbortController.abort();
  entityIconAbortController = new AbortController();
  if (typeof closeEntityOverlay === 'function') closeEntityOverlay();
  ...
  try {
    ...
    try {
      const storedRes = await fetch(
        `${MY_WORKER_URL}/api/etf-holdings-stored?symbol=${encodeURIComponent(ticker)}&meta=fc`,
      ...
      if (storedRes.ok) {
        ...
        if (stored.holdings && stored.holdings.length > 0) {
          data = {
            ...
            // A4: pass freshness fields through to UI
            holdings_as_of: stored.holdings_as_of || null,
            data_freshness: stored.data_freshness || null,
            ...
            holdings: stored.holdings.map(h => ({
              ...
              // A2: asset type for bond detection; coupon/maturity/issuer not in D1
              assetCat:      h.asset_cat,
              asset_type:    h.asset_type || h.asset_cat,
              assetType:     h.asset_type || h.asset_cat,
              issuerName:    h.issuer_name || null,
              couponKind:    null,
              annualizedRte: null,
              maturityDat:   h.maturity_date || null,
              yieldVal:      null,
              country:       h.issuer_country
            })),
            fundInfo: {
              regName:   stored.name,
    ...
    const holdingsCount  = currentEtfHoldings.length;  // A1: from array length
    ...
    // A4: freshness from stored response
    const holdingsAsOf   = data.holdings_as_of || null;
    const dataFreshness  = data.data_freshness || null;
    // v2: freshness chip above the holdings table
    const holdingsFcMeta = data.meta || null;
    const freshnessBadge = (holdingsAsOf || dataFreshness) ? _etfFreshnessBadge(dataFreshness || 'current', holdingsAsOf) : '';
    const holdingsFreshnessChipHtml = holdingsFcMeta
      ? `<div style="margin-bottom:6px;">${renderFreshnessChip(holdingsFcMeta.freshness.status, holdingsFcMeta.freshness.asOfDate)}</div>`
      : '';
    ...
    contentDiv.innerHTML = `
      ...
      <div style="padding:12px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--r);margin-bottom:10px;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;border-top:1px solid var(--border);padding-top:8px;font-size:10.5px;">
          ...
          ${freshnessBadge ? `<div style="display:flex;flex-direction:column;gap:2px;justify-content:flex-end;">
            ${freshnessBadge}
          </div>` : ''}
      ...
      <!-- HOLDINGS FRESHNESS CHIP (v2) -->
      ${holdingsFreshnessChipHtml}
...
function _showHoldingsContent() {
  ...
  if (tbody) {
    ...
    // Holdings DOM was destroyed by another tab — re-fetch
    const contentDiv = document.getElementById('etfHoldingsContent');
    const ticker = contentDiv?.dataset.activeTicker;
    if (ticker) onEtfSelectChange(ticker);
  }
}
...
async function _renderOverviewTab(ticker) {
  ...
  try {
    const res = await fetch(
      `${MY_WORKER_URL}/api/etf-snapshot?symbol=${encodeURIComponent(ticker)}&meta=fc`
    );
    if (!res.ok) throw new Error(`Snapshot failed: ${res.status}`);
    const data = await res.json();
    const snaps = data.snapshots || [];
    const etf   = currentEtfsList.find(e => e.ticker === ticker) || {};
    // A4: freshness from snapshot response
    const ovFreshness = data.data_freshness || null;
    const ovAsOf      = data.holdings_as_of || null;
    const ovFreshBadge = (ovFreshness || ovAsOf) ? _etfFreshnessBadge(ovFreshness || 'current', ovAsOf) : '';
    // v2: freshness + coverage chips strip
    const fcMeta = data.meta || null;
    const ovChipsHtml = fcMeta
      ? `<div style="margin-bottom:8px;display:flex;align-items:center;gap:4px;flex-wrap:wrap;">
           ${renderEtfBadge(fcMeta)}
         </div>`
      : '';
    ...
    contentDiv.innerHTML = `
      ...
      <div style="padding:12px;background:var(--bg3);border:1px solid var(--border);
        ...
        <div style="font-size:12px;color:var(--text2);margin-bottom:4px;">${etf.name || ''}</div>
        ${ovChipsHtml}
        <div style="display:flex;flex-wrap:wrap;gap:12px;font-size:10.5px;
          ...
          ${ovFreshBadge ? `<div style="display:flex;align-items:flex-end;">${ovFreshBadge}</div>` : ''}
          ${hasDerivatives ? `
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;font-size:11px;">
          ...
          <div style="display:flex;flex-direction:column;gap:2px;">
            ...
            <span style="font-size:9px;padding:2px 7px;border-radius:3px;
              ...
              ${(etf.coverageStatus || '').toLowerCase() === 'deep' ? 'DEEP' : 'DIRECTORY'}
            </span>
          </div>
        </div>
      </div>`;
...
async function _renderChangesTab(ticker) {
  ...
  try {
    const res = await fetch(
      `${MY_WORKER_URL}/api/etf-changes?symbol=${encodeURIComponent(ticker)}&meta=fc`
    );
    if (!res.ok) throw new Error(`Changes fetch failed: ${res.status}`);
    const data = await res.json();

    // v2: single-month notice
    const changesFcMeta = data.meta || null;
    if (changesFcMeta?.coverage?.depth === 'single_month') {
      contentDiv.innerHTML = `
        <div style="padding:12px 16px;background:var(--bg3);border:1px solid var(--border);
                    border-radius:var(--r);margin-bottom:12px;font-size:11px;color:var(--dim);">
          Only one filing month stored — changes view requires 2+ months
        </div>`;
      return;
    }
...
function updateHoldingsDisplay() {
  ...
  // Headers
  const thBase = 'position:sticky;top:0;background:var(--bg3);z-index:10;';
  if (isBond) {
    // A2 revised bond layout: Security name | CUSIP | Asset type | Country | Weight %
    // Coupon / maturity / issuer not in D1 — omitted entirely
    thead.innerHTML = `<tr>
      <th style="text-align:left;${thBase}">Security name</th>
      <th style="text-align:left;width:100px;font-family:var(--mono);${thBase}">CUSIP</th>
      <th style="text-align:left;width:110px;${thBase}">Asset type</th>
      <th style="text-align:center;width:50px;${thBase}">Ctry</th>
      <th style="text-align:right;width:70px;${thBase}">Weight %</th>
    </tr>`;
  } else {
    thead.innerHTML = `<tr>
      <th style="text-align:left;width:120px;font-family:var(--mono);${thBase}">ISIN</th>
      <th style="text-align:left;width:60px;${thBase}">Ticker</th>
      <th style="text-align:left;${thBase}">Security Name</th>
      <th style="text-align:left;width:110px;${thBase}">Asset type</th>
      ${showCountry ? `<th style="text-align:center;width:50px;${thBase}">Ctry</th>` : ""}
      <th style="text-align:right;width:65px;${thBase}">Weight %</th>
    </tr>`;
  }
  ...
  let rows = "";
  if (sliced.length === 0) {
    // bond: 5 cols; equity: 5 with country, 5 without (isin/ticker/name/assettype/weight)
    const cols = isBond ? 5 : (showCountry ? 6 : 5);
    ...
    sliced.forEach(h => {
      ...
      // A2: readable asset type label for both layouts
      const rawAt      = h.assetType || h.asset_type || h.assetCat || h.asset_cat || '';
      const atLabel    = _assetCatLabel(rawAt);

      if (isBond) {
        // A2 revised bond layout: Security name | CUSIP | Asset type | Country | Weight %
        const nameStr = h.name || "—";
        const cusip   = h.cusip || "—";
        const ctry    = h.country || h.issuer_country || "";
        const ctryCell = ctry
          ? `<td style="text-align:center;"><span style="font-size:9px;font-weight:700;font-family:var(--mono);background:var(--bg3);border:1px solid var(--border);border-radius:3px;padding:1px 4px;">${ctry}</span></td>`
        ...
        rows += `<tr>
          ...
          <td style="font-family:var(--mono);font-size:10px;color:var(--dim);">${cusip}</td>
          <td style="font-size:10px;color:var(--text2);">${atLabel}</td>
          ${ctryCell}
          <td ${wStyle}>${wStr}</td>
        </tr>`;
      } else {
        ...
        const nameStr = h.name || "—";
        const entityIconHtml = h.isin
          ? `<span class="entity-link-icon" data-isin="${h.isin}" title="View entity — ${h.isin}"
               onclick="handleEntityIconClick(this,'${h.isin}')"
               style="color:#4DC8C8;font-size:0.7em;font-weight:700;margin-left:4px;vertical-align:super;cursor:pointer;user-select:none;opacity:0.9;">e</span>`
          : '';
        rows += `<tr>
          <td>${isin}</td>
          <td>${tkrCell}</td>
          <td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${nameStr}">${nameStr}${entityIconHtml}</td>
          <td style="font-size:10px;color:var(--text2);">${atLabel}</td>
          ${countryCellHtml}
          <td ${wStyle}>${wStr}</td>
        </tr>`;
...
window.runEtfOverlap = async function() {
  ...
  try {
    const res = await fetch(
      `${MY_WORKER_URL}/api/etf-overlap?a=${encodeURIComponent(tickerA)}&b=${encodeURIComponent(tickerB)}&meta=fc`
    );
    const data = await res.json();
    ...
    results.innerHTML = `
      ...
      <div style="text-align:center;padding:24px;background:var(--bg3);
        ...
        ${data.etf_a.meta || data.etf_b.meta ? `
        <div style="margin-top:8px;display:flex;justify-content:center;gap:16px;font-size:10px;color:var(--dim);">
          ${data.etf_a.meta ? `<span>${data.etf_a.ticker}: ${renderEtfBadge(data.etf_a.meta)}</span>` : ''}
          ${data.etf_b.meta ? `<span>${data.etf_b.ticker}: ${renderEtfBadge(data.etf_b.meta)}</span>` : ''}
        </div>` : ''}
      </div>
...
window.runEtfCompare = async function() {
  ...
  try {
    const res = await fetch(
      `${MY_WORKER_URL}/api/etf-compare?symbols=${encodeURIComponent(symbols.join(','))}&meta=fc`
    );
    const data = await res.json();
    if (data.error) {
    ...
    const headerHtml = etfs.map(e => `
      <div style="padding:12px;background:var(--bg3);border:1px solid var(--border);
        ...
        ${e.meta
          ? `<div style="margin-top:4px;">${renderFreshnessChip(e.meta.freshness.status, e.meta.freshness.asOfDate)}</div>`
          : (e.coverage_status||'').toLowerCase()==='deep'
...
window.runExposureSearch = async function() {
  ...
  try {
    const url = mode === 'stock'
      ? `${MY_WORKER_URL}/api/etf-exposure?stock=${encodeURIComponent(query)}&meta=fc`
      : `${MY_WORKER_URL}/api/etf-exposure?country=${encodeURIComponent(query)}&meta=fc`;
    ...
    // v2: freshness context chip on ETF context line
    const exposureFreshnessHtml = data.meta
      ? `<div style="margin-bottom:8px;">${renderFreshnessChip(data.meta.freshness.status, data.meta.freshness.asOfDate)}<span style="font-size:10px;color:var(--dim);margin-left:6px;">Latest holdings data</span></div>`
      : '';

    // A3: alias resolution banner
    const aliasBannerHtml = data.alias_used
      ? `<div style="padding:8px 12px;margin-bottom:12px;background:rgba(59,130,246,0.1);
           border:1px solid var(--blue);border-radius:var(--r);font-size:11px;color:var(--blue);">
           Showing results for <strong>${data.resolved_to}</strong>
           (matched from '${query}')
         </div>`
      : '';

    if (!data.etfs || data.etfs.length === 0) {
      const noResultMsg = data.alias_used
        ? `No ETFs found holding <strong>${data.resolved_to}</strong> (matched from '${query}'). Try searching by a different name or check the spelling.`
        : `No ETFs found with exposure to <strong>${query}</strong> in the latest holdings data (${data.report_month}).`;
      results.innerHTML = `${aliasBannerHtml}
        <div style="padding:32px;text-align:center;color:var(--dim);font-size:11px;">${noResultMsg}</div>`;
      return;
    }
...
window.loadUniverseChanges = async function() {
  ...
  try {
    let url = `${MY_WORKER_URL}/api/universe-changes?scope=${scope}&meta=fc`;
    ...
    // v2: aggregate freshness count from meta
    const universeFreshMeta = data.meta || null;
    const universeFreshnessSummaryHtml = universeFreshMeta
      ? `<div style="padding:8px 14px;background:var(--bg3);border:1px solid var(--border);border-radius:var(--r);">
           <div style="font-size:9px;color:var(--dim);">Data Freshness</div>
           <div style="font-size:12px;margin-top:2px;">${renderFreshnessChip(universeFreshMeta.freshness.status, universeFreshMeta.freshness.asOfDate)}</div>
         </div>`
      : '';
...
// ── Entity icon click handler ──────────────────────────────────
window.handleEntityIconClick = function(el, isin) {
  if (!isin) return;
  ...
  const ENTITIES_API = 'https://meridian-entities-api.navinmv1981.workers.dev';

  fetch(`${ENTITIES_API}/api/entities/isin/${encodeURIComponent(isin)}`,
    { signal: entityIconAbortController?.signal })
    .then(res => {
    ...
    .then(data => {
      ...
      if (!entity || !entity.entity_id) throw new Error('no_entity');
      ...
      // Show entity detail as overlay ON TOP of ETF panel
      // Do NOT close the ETF panel
      showEntityOverlay(entity.entity_id);
    })
    .catch((err) => {
      if (err.name === 'AbortError') return; // cancelled by ETF switch, not a real failure
      // No entity found — grey icon permanently
      el.textContent = 'e';
      el.style.opacity = '0.2';
      el.style.pointerEvents = 'none';
      el.title = 'Entity not yet resolved';
